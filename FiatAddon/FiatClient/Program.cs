  ...
  HaDevice haDevice)
{
  ...
  var engineSwitch = new HaSwitch(mqttClient, "EngineStart", haDevice, async sw =>
  {
    if (await TrySendCommand(fiatClient, sw.IsOn ? FiatCommand.REON : FiatCommand.REOFF, vehicle.Vin))
      forceLoopResetEvent.Set();
  });
  return new HaEntity[]
  {
